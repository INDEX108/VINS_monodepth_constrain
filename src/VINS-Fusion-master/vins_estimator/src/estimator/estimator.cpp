...
// {
//     inputImageCnt++;
//     map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> featureFrame;   // feature_id  camera_id  x, y, z, p_u, p_v, velocity_x, velocity_y;
...
void Estimator::inputImage_depth(double t, const cv::Mat &_img, const cv::Mat &_img_depth)
{
    inputImageCnt++;
    map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> featureFrame;   // feature_id  camera_id  x, y, z, p_u, p_v, velocity_x, velocity_y;
    TicToc featureTrackerTime;

    featureFrame = featureTracker.trackImage_depth(t, _img, _img_depth);
    //printf("featureTracker time: %f\n", featureTrackerTime.toc());
    ...
    if (SHOW_TRACK)
    {
        cv::Mat imgTrack = featureTracker.getTrackImage();
        pubTrackImage(imgTrack, t);
        cv::Mat imgTrack_depth = featureTracker.getTrackImage_depth();
        pubTrackImage_depth(imgTrack_depth, t);
    }
...
void Estimator::inputFeature(double t, const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &featureFrame)  //该函数并未使用
{
    mBuf.lock();
    featureBuf.push(make_pair(t, featureFrame));
    mBuf.unlock();
...
void Estimator::processMeasurements()  //传感器数据处理入口，也是多线程配置中传入的回调函数
{
    while (1)
    {
        ...
        pair<double, map<int, vector<pair<int, Eigen::Matrix<double, 8, 1> > > > > feature; //时间戳（double）、路标点编号（int）、相机编号（int）
                                        //特征点信息（Matrix<double, 8, 1>，归一化相机坐标系坐标（3维）、去畸变图像坐标系坐标（2维）、特征点速度（2维）深度估计（1维）
...
void Estimator::optimization()
{
    ...
    {
        problem.AddParameterBlock(para_Sk[i], SIZE_SCALE_SHIFT);  //q、p参数
    }
    //构建残差
    /*******先验残差*******/
    ...
    /*******单目深度残差*******/
    //单目深度残差相关，此时使用了Cauchy损失核函数
    ceres::LossFunction *loss_function_dps;
    loss_function_dps = new ceres::CauchyLoss(1.0);  //Cauchy损失核函数
    ceres::LossFunction *loss_function_sc;
    loss_function_sc = new ceres::HuberLoss(1.0);  //Cauchy损失核函数
    int f_sk_cnt = 0;
    feature_index = -1;
    for (auto &it_per_id : f_manager.feature)  //遍历路标点
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)  //遍历观测到路标点的图像帧
        {
            ...
            if (imu_i == imu_j) {
                ...
                Self_Scale *ssc = new Self_Scale(it_per_frame.depth);
                problem.AddResidualBlock(ssc, loss_function_sc, para_Feature[feature_index], para_Sk[imu_i]);
            }
            if (imu_i != imu_j)
            {
                Vector3d pts_j = it_per_frame.point;  //测量值
                //左相机在i时刻和j时刻分别观测到路标点，计算j时刻的前向传播深度和depthnet预测
                Scale_shift *sc = new Scale_shift(pts_i, pts_j, it_per_id.feature_per_frame[0].velocity, it_per_frame.velocity,
                                                                 it_per_id.feature_per_frame[0].cur_td, it_per_frame.cur_td,it_per_frame.depth);
                problem.AddResidualBlock(sc, loss_function_dps, para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0], para_Feature[feature_index], para_Td[0],para_Sk[imu_j]);
            }
    ...
    //ROS_DEBUG("SK measurement count: %d", f_sk_cnt);


    //优化参数配置
    for (int i = 0; i < frame_count + 1; i++) {
        problem.SetParameterUpperBound(para_Sk[i], 0, 5.0);
        problem.SetParameterLowerBound(para_Sk[i], 0, 0.1);
        problem.SetParameterUpperBound(para_Sk[i], 1, 1.0);
        problem.SetParameterLowerBound(para_Sk[i], 1, 0.0);
    }
    ...
    outfile.open("/home/index108/VINS/VINS_FUSION/extrinsic.txt", ios::app);
